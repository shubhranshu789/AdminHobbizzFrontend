...
interface News {
 ...
 function UpdateNewsinner() {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photoartNews/${newsId}`)
        if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photoclub-news-update/${newsId}`, {
        method: "PUT",
        headers: {
