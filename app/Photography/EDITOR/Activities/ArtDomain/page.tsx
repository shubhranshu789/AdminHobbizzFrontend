...
const ClubNewsForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photoclubdomain`, {
        method: "POST",
  ...
  const fetchNews = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photoclubdomainviewallpost`);
      const data = await res.json();
  ...
  const handleDelete = async (id: any) => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photoclub-domain/${id}`, {
        method: "DELETE",
        headers: {
