...
//   const { toast } = useToast()
  ...
  useEffect(() => {
    const fetchDomain = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photoDomain/${domainId}`)
        if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photoclub-domain-update/${domainId}`, {
        method: "PUT",
        headers: {
