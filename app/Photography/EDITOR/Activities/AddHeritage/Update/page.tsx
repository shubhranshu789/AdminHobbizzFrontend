...
interface Heritage {
 ...
 function UpdateHeritageinner() {
  ...
  useEffect(() => {
    const fetchHeritage = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photoHeritage/${heritageId}`)
        if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photoclub-heritage-update/${heritageId}`, {
        method: "PUT",
        headers: {
