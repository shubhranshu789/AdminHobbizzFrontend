...
interface Legacy {
 ...
 function UpdateLegacyinner() {
  ...
  useEffect(() => {
    const fetchLegacy = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photoLEGACY/${legacyId}`)
        if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photoclub-legacy-update/${legacyId}`, {
        method: "PUT",
        headers: {
