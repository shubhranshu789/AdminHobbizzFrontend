...
const [openImage, setOpenImage] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/photogetCompitition/${id}`, {
      headers: {
  ...
  useEffect(() => {
    const fetchUploadStatus = async () => {
      ...
      try {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photohas-uploaded-compitition/${id}`, {
          headers: {
  ...
  useEffect(() => {
    const fetchParticipants = async () => {
      try {
        const token = localStorage.getItem("jwt");
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photoevent-participants-compi/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const registerForActivity = async (activityId: string) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photoregister-compitition/${activityId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const unregisterFromActivity = async (activityId: string) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photounregister-compitition/${activityId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const post = async () => {
    ...
    try {
      ...
      if (result.url) {
        const token = localStorage.getItem("jwt");
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/photoupload-photo-compitition/${id}`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
